...
export default function FounderWisdom() {
  ...
  return (
    <section id="wisdom" className="py-24 relative bg-[#F8FAFC] border-t border-slate-200/80 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-[#2BF4C3]/5 rounded-full blur-[170px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white border border-slate-200 mb-3.5 shadow-sm">
            <Lightbulb className="w-3.5 h-3.5 text-[#00C49F]" />
            <span className="text-xs font-bold uppercase tracking-wider text-slate-800">
              Founder Principles &amp; Heuristics
            </span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight">
            Unfiltered <span className="text-[#00C49F]">Founder Wisdom</span>
          </h2>
        ...
        <div className="bg-white rounded-3xl p-8 sm:p-12 border border-slate-200 shadow-xl relative mb-10 overflow-hidden max-w-4xl mx-auto">
          {/* Top logo mint accent bar */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-[#2BF4C3]" />
          
          {/* Subtle background quote watermark */}
          <div className="absolute -top-4 right-6 text-[#2BF4C3]/10 pointer-events-none select-none">
            <Quote className="w-40 h-40" />
          </div>

          <div className="relative z-10">
            <div className="flex flex-wrap items-center gap-2 mb-6">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-[#2BF4C3]/10 text-slate-900 border border-[#2BF4C3]/30 shadow-sm">
                <Sparkles className="w-3 h-3 text-[#00C49F]" />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6 border-t border-slate-100">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-slate-900 text-[#2BF4C3] flex items-center justify-center font-bold text-xs shadow-sm">
                  MD
                </div>
                <div>
              ...
              >
                {copied ? (
                  <>
                    ...
                    <span className="text-slate-900 font-semibold">Copied to Clipboard</span>
                  </>
                ) : (
                  <>
          ...
          {FOUNDER_WISDOM.map((item, idx) => (
            <button
              ...
              className={`p-5 rounded-2xl text-left transition-all cursor-pointer border ${
                activeIdx === idx
                  ? "bg-white border-[#2BF4C3] shadow-md ring-2 ring-[#2BF4C3]/20 scale-[1.02]"
                  : "bg-white/80 border-slate-200 hover:border-slate-300 text-slate-600 hover:bg-white shadow-sm"
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                <span className={`text-xs font-bold uppercase tracking-wider ${activeIdx === idx ? "text-[#00C49F]" : "text-slate-500"}`}>
                  {item.theme}
                </span>
